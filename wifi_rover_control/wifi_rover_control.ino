...
// Fine trim for residual straight-line drift (left wheel now has real PWM on D3).
// Start at 1.0; lower it if it veers right, raise toward 1.0 if it veers left.
const float LEFT_TRIM = 1.0;

void setMotor(int dirPin, int enPin, int speed) {
  if (enPin == EN_LEFT) speed = (int)(speed * LEFT_TRIM);
  if (speed == 0) {
    analogWrite(enPin, 0);
  } else if (speed > 0) {
